...
class App extends Component {
  ...
  inputChangedHandler = (event) => {
    ...
    this.setState({ inputText: inputText, inputLength: inputLength });
  };
  ...
  onDeleteHandler = (event, index) => {
    ...
    this.setState({ inputLength: newInput.length });
  };
  render() {
    let chars = null;
    chars = (
      <div>
        {[...this.state.inputText].map((l, index) => {
          return (
            <CharComponent
              click={(event) => this.onDeleteHandler(event, index)}
              letter={l}
              key={index}
            />
          );
